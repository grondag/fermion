...
public class Useful {
	...
	static {
		...
		for (final long packed : Useful.fill2dCircleInPlaneXZ(DISTANCE_SORTED_CIRCULAR_OFFSETS_MAX_RADIUS).toLongArray()) {
			...
			final int z = PackedBlockPos.getZ(packed);
	...
	public static LongArrayList line2dInPlaneXZ(long packedPos1, long packedPos2) {
		final int x1 = PackedBlockPos.getX(packedPos1);
		final int z1 = PackedBlockPos.getZ(packedPos1);

		final int x2 = PackedBlockPos.getX(packedPos2);
		final int z2 = PackedBlockPos.getZ(packedPos2);

		final int dx = Math.abs(x2 - x1);
		final int dz = Math.abs(z2 - z1);

		final LongArrayList result = new LongArrayList(Math.max(dx, dz) * 3 / 2);

		final int sx = x1 < x2 ? 1 : -1;
		final int sysz = z1 < z2 ? 1 : -1;
