...
public class VoxelBitField {
    ...
    public double getInflatedVolume() {
    ...
    public double getDiscardedVolume() {
    ...
    /**
     * Power is how many bits per dimension. Equates to powers of 2.
     */
        ...
        for (int i = 0; i < retVal.getBitsPerAxis(); i++) {
            for (int j = 0; j < retVal.getBitsPerAxis(); j++) {
                for (int k = 0; k < retVal.getBitsPerAxis(); k++) {
                    int fillCount = 0;
                    if (isFilled(i * 2, j * 2, k * 2))
                        fillCount++;
                    if (isFilled(i * 2, j * 2, k * 2 + 1))
                        fillCount++;
                    if (isFilled(i * 2, j * 2 + 1, k * 2))
                        fillCount++;
                    if (isFilled(i * 2, j * 2 + 1, k * 2 + 1))
                        fillCount++;
                    if (isFilled(i * 2 + 1, j * 2, k * 2))
                        fillCount++;
                    if (isFilled(i * 2 + 1, j * 2, k * 2 + 1))
                        fillCount++;
                    if (isFilled(i * 2 + 1, j * 2 + 1, k * 2))
                        fillCount++;
                    if (isFilled(i * 2 + 1, j * 2 + 1, k * 2 + 1))
                        fillCount++;
    ...
    public void setOrigin(int x, int y, int z) {
        ...
        ArrayList<DistanceRankedVoxel> distances = new ArrayList<DistanceRankedVoxel>(
                getBitsPerAxis() * getBitsPerAxis() * getBitsPerAxis());
    ...
    /**
     * True if all bits that were true at origin set appear to have been consumed
     * via bounding box creation.
     */
    ...
    /**
     * Creates a 1-voxel box at a filled voxel nearest the origin. The bit in this
     * voxel is set unfilled when it is created.
     * 
     * Assumes that voxels that no more voxels are filled after origin is set.
     * (Voxels are only unfilled after that, as boxes are created that consume
     * them.) Returns null if no more filled voxels.
     */
        ...
        for (int z = clamp(minZ); z <= clamp(maxZ); z++) {
            for (int y = clamp(minY); y <= clamp(maxY); y++) {
                ...
                for (int x = clamp(minX); x <= clamp(maxX); x++) {
                    // retVal = retVal && bits.get(getAddress(clamp(minX), y, z),
                    // getAddress(clamp(maxX), y, z) + 1).cardinality() == (clamp(maxX) -
                    // clamp(minX) + 1);
    ...
    public class VoxelBox {
        ...
        public boolean canExpand(Direction face) {
            return expandify(face, false);
        }

        /**
         * The actual guts of canExpand and expandIfPossible. Handles the testing and
         * bit flipping. Doesn't actually change the bounding values of the box. That is
         * done by expandIfPossible.
         * 
         * Could probably make this much cleaner if I used vectors of some kind but
         * sometimes I enjoy being old fashioned.
         */
        private boolean expandify(Direction face, boolean doit) {
            int minTestX = minX;
            int minTestY = minY;
            int minTestZ = minZ;
            int maxTestX = maxX;
        ...
        public boolean expandIfPossible(Direction face) {
