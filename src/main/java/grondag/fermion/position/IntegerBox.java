...
public class IntegerBox {
	...
	public static class Builder {
		...
		public void add(int x1, int y1, int z1, int x2, int y2, int z2) {
			if (seenFirst) {
				minX = Useful.min(minX, x1, x2);
				minY = Useful.min(minY, y1, y2);
				minZ = Useful.min(minZ, z1, z2);
				maxX = Useful.max(maxX, x1, x2);
				maxY = Useful.max(maxY, y1, y2);
				maxZ = Useful.max(maxZ, z1, z2);
			} else {
				seenFirst = true;
				minX = Math.min(x1, x2);
				minY = Math.min(y1, y2);
	...
	public boolean equals(Object other) {
		if (other == null)
			...
			final IntegerBox otherBox = (IntegerBox) other;
