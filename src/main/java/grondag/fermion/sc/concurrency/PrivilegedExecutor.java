...
public class PrivilegedExecutor extends ThreadPoolExecutor
{
	...
	{
		final RunnableFuture<T> ftask = new PrivilegedFutureTask<T>(task, isPrivileged);
		super.execute(ftask);
		return ftask;
	}
	...
	{
		final RunnableFuture<T> ftask = new PrivilegedFutureTask<T>(task, result, isPrivileged);
		super.execute(ftask);
		return ftask;
	}
	...
	{
		final RunnableFuture<Void> ftask = new PrivilegedFutureTask<Void>(task, null, isPrivileged);
		super.execute(ftask);
		return ftask;
	}
