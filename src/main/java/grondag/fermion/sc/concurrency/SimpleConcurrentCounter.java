...
public class SimpleConcurrentCounter
{
	...
	private final ThreadLocal<LongHolder> start = new ThreadLocal<LongHolder>()
	{
		@Override
	...
	public void endRun()
	{
		final long nanos = runTime.addAndGet(System.nanoTime() - start.get().start);
		final int samples = runCount.incrementAndGet();
