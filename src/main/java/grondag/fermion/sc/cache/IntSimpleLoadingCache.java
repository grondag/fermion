...
public class IntSimpleLoadingCache<V> implements ISimpleLoadingCache
{
	...
	{
		final IntCacheState<V> localState = activeState;
		...
		int position = HashCommon.mix(key) & positionMask;
	...
	protected V load(IntCacheState<V> localState, int key, int position)
	{
		...
		final IntCacheState<V> backupState = this.backupState.get();
		...
		{
			final IntCacheState<V> newState = new IntCacheState<V>(this.capacity);
