...
public class WideSimpleLoadingCache<V> implements ISimpleLoadingCache
{
	...
	{
		final WideCacheState<V> localState = activeState;
		...
		int position = (((int)HashCommon.mix(key1) ^ (int)HashCommon.mix(key2)) * 2) & positionMask;
	...
	protected V load(WideCacheState<V> localState, long key1, long key2, int position)
	{
		...
		final WideCacheState<V> backupState = this.backupState.get();
		...
		{
			final WideCacheState<V> newState = new WideCacheState<V>(this.capacity);
