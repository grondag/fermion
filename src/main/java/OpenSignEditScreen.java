...
//
//		addButton(new ButtonWidget(width / 2 - 100, height / 4 + 120, 200, 20, new TranslatableText("gui.done"), (widget) -> {
//			finishEditing();
//		}));
//
...
//		}, (string) -> {
//			sign.setTextOnRow(currentRow, new LiteralText(string));
//		}, SelectionManager.makeClipboardGetter(client), SelectionManager.makeClipboardSetter(client), (string) -> {
//			return client.textRenderer.getWidth(string) <= 90;
//		});
//	}
//
//	@Override
...
//	}
//
//	// FIX: make this mirror newer Mojang logic?  Or use access wideners?
//	@Override
//	public void render(MatrixStack matrixStack, int i, int j, float tickDelta) {
//		this.renderBackground(matrixStack);
//		drawCenteredText(matrixStack, textRenderer, title, width / 2, 40, 16777215);
//		matrixStack.push();
...
//				return list.isEmpty() ? "" : list.get(0).getString().asFormattedString();
//			});
//		}
//
...
//			if (line != null) {
//				final float s = -client.textRenderer.getWidth(line) / 2;
...
//					final int x = client.textRenderer.getWidth(line.substring(0, Math.max(Math.min(n, line.length()), 0)));
//					final int y = (x - client.textRenderer.getWidth(line) / 2) * p;
//
//					if (n >= line.length()) {
...
//				final int w = client.textRenderer.getWidth(line.substring(0, Math.max(Math.min(n, line.length()), 0)));
//				final int x = (w - client.textRenderer.getWidth(line) / 2) * p;
//
//				if (fade && n < line.length()) {
//					final int var34 = q - 1;
//					final int var10003 = x + 1;
//					client.textRenderer.getClass();
//					super.fill(matrixStack, x, var34, var10003, q + 9, -16777216 | l);
//				}
//
//				if (o != n) {
//					final int y = Math.min(n, o);
//					final int z = Math.max(n, o);
//					final int aa = (client.textRenderer.getWidth(line.substring(0, y)) - client.textRenderer.getWidth(line) / 2) * p;
//					final int ab = (client.textRenderer.getWidth(line.substring(0, z)) - client.textRenderer.getWidth(line) / 2) * p;
...
//
//		matrixStack.pop();
//		super.render(matrixStack, i, j, tickDelta);
//	}
//}
